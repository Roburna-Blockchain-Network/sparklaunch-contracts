...
contract SparklaunchSaleTest {
    ...
    function grantATierMultiply(address[] memory addys, uint256[] memory tiers) external {
    ...
    function grantATier(address user, uint256 _tier) public {
    ...
    function editMaxAndMinParticipation(uint256 _maxP, uint256 _minP) external{
        ...
        external
    {
        ...
        require(publicRoundStartDelta + startTimes[4] < sale.saleEnd, "Public round should start before sale end");

        uint256 lastTimestamp = 0;

        require(startTimes[0] >= block.timestamp, "Start time < current time");
